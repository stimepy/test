...
if (isset($_POST['del_existing']) && $_POST['lang_choices']) {
    ...
    foreach ($tabs as $del_table) {
        ...
        echo $qry;
            $message .= ($sql->db_QueryFreeForm($qry)) ? $_POST['lang_choices']." ".$del_table." deleted<br />" :
             $_POST['lang_choices']." $del_table couldn't be deleted<br />";//mysql_query($qry)
        }
    }
    global $cachevar;
...
if (isset($_POST['kreate_tbl']) && $_POST['language']) {
    ...
    foreach ($tabs as $value) {
        ...
        if (isset($_POST[$value])) {
            ...
            if ($_POST['remove']) {
                // Remove table.
                $message .= ($sql->db_QueryFreeForm("DROP TABLE ".$mySQLprefix."lan_".$lang."_".$value)) ? $_POST['language']." ".$value." ".LAN_DELETED."<br />" :  $_POST['language']." $value ".LANG_LAN_02."<br />";//$mysql_query
            } else {
                // leave table.
...
function db_Table_exists($table)
{
    ...
    {
        //$tablist
        $tables = $sql->getTablelist();//mysql_list_tables($mySQLdefaultdb);
...
function copy_table($oldtable, $newtable, $drop = FALSE, $data = FALSE)
{
    ...
    $sql->db_QueryFreeForm('SET SQL_QUOTE_SHOW_CREATE = 1');
...
//        $qry = str_replace($old, $new, $qry);
    ...
    $result = $sql->db_QueryFreeForm($qry);
    if(!$result)
    {
        return FALSE;
    }
