		...
		while (list($key, $image_name) = each($dirlist)) {
			...
			if ($sql->db_Select("user", "*", "user_image='-upload-$image_name' OR user_sess='$image_name'")) {
				while ($row = $sql->db_Fetch()) {
					extract($row);
					$users .= e107UserUrl($user_id, $user_name)."<span class='smalltext'>(".($user_sess == $image_name ? IMALAN_24 : IMALAN_23).")</span> | ";
				}
			} else {
				$users = IMALAN_22;
			}
...
if (isset($_POST['check_avatar_sizes'])) {
	...
	//
			if (strlen($sBadImage))
			{
				...
				<td class='forumheader3'>".IMALAN_51. e107UserUrl($user_id, $user_name) .$sBadImage."</td>
				</tr>";
			}
		}
	}
