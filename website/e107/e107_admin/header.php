...
function admin_update($update, $type = 'update', $success = false, $failed = false) {
	...
	if (($type == 'update' && $update) || ($type == 'insert' && $update !== false)) {
	...
	} else if (!$sql->getSqlErrorno()) {//mysql_errno()
		if ($type == 'update') {
		...
		$caption = LAN_UPDATED_FAILED;
		$text = "<b>".($failed ? $failed : LAN_UPDATED_FAILED." - ".LAN_TRY_AGAIN)."</b><br />".LAN_ERROR." ".$sql->getSqlErrorno().": ".$sql->getSqlErrorno();//mysql_errno()
	}
