...
function get_current($tab, $prefix = "") 
{
  ...
  $result = $sql->db_QueryFreeForm('SET SQL_QUOTE_SHOW_CREATE = 1'); //mysql_query('SET SQL_QUOTE_SHOW_CREATE = 1');
  $qry = 'SHOW CREATE TABLE `'.$prefix.$tab."`";
  $z = $sql->db_QueryFreeForm($qry);//mysql_query($qry);
  if ($z) 
  {
	$row = $sql->db_Fetch();//mysql_fetch_row($z);
...
if(isset($_POST['do_fix']))
	...
	foreach( $_POST['fix_active'] as $key=>$val)
	{
		...
		$text .= ($sql->db_QueryFreeForm($query)) ? " - <b>".LAN_UPDATED."</b>" : " - <b>".LAN_UPDATED_FAILED."</b>"; //mysql_query($query)
		$text .= "</td></tr>";
...
function table_list() {
    ...
    $prefix = MPREFIX."lan_";
	 foreach($tables as $temp){//while   mysql_fetch_array($tables)){
