...
if(!defined("e_TRACKING_DISABLED") && (isset($pref['track_online']) && $pref['track_online'])) {
	...
	{
      $membership=$e107cache->retrieve("nq_user_totals", 120);
      if(!$membership){
	    $total_members = $sql->db_Count("user","(*)","where user_ban = 0 and (user_id>53 or user_id=2)");
      }
       else{
           $total_members=$total_membership;
       }
	  $newest_member = $sql->db_Select("user", "user_id, user_name", "user_ban='0' and (user_id>53 or user_id=2) ORDER BY user_join DESC LIMIT 0,1");
	  $row = $sql->db_Fetch();
	  extract($row);
	  $members_totals = "<br />".ONLINE_L5.": ".$total_members.", ".ONLINE_L6.": ".e107UserUrl($user_id,$user_name);
