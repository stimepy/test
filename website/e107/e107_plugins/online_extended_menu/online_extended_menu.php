...
if(!defined("e_TRACKING_DISABLED") && (isset($pref['track_online']) && $pref['track_online'])) 
{
	...
	if (MEMBERS_ONLINE) 
	{
		...
		foreach($listuserson as $uinfo => $pinfo) {
			...
			$text .= "<img src='".e_IMAGE_ABS."admin_images/users_16.png' alt='' style='vertical-align:middle' /> ". e107UserUrl($oid,$oname) . ONLINE_EL7;
	...
	$text .= "<br />".ONLINE_EL8." ".($menu_pref['most_members_online'] + $menu_pref['most_guests_online'])."<br />(".ONLINE_EL2.$menu_pref['most_members_online'].", ".ONLINE_EL1.$menu_pref['most_guests_online'].") ".ONLINE_EL9." ".$datestamp."<br />";
 	...
 	$total_members = $sql->db_Count("user","(*)","where user_ban = 0  and (user_id>53 or user_id=2)");
	if ($total_members > 1) {
		$newest_member = $sql->db_Select("user", "user_id, user_name", "user_ban = 0 and user_id>53 ORDER BY user_join DESC LIMIT 1");
		$row = $sql->db_Fetch();
		extract($row);
		$text .= "<br />".ONLINE_EL5.": ".$total_members."<br />".ONLINE_EL6.": ".e107UserUrl($user_id,$user_name);
	}
} elseif(ADMIN) {
	global $tp;
