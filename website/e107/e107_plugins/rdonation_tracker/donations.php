...
$mc_currency = dollar_rep($pref['rdtrack_currency']);
...
$text .= "
    ...
    foreach ($acur1 as $key => $color) {
      $key2 = $color[1];
      $text .= "<option ".((strcmp($mc_currency, $key)==0) ? " selected ='selected'" : "")." value='".$key2."'>".$color[0]." ".$key."</option>";
    }
