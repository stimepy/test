...
function make_hourmin($boxname,$cur_hour,$cur_minute)
{
  ...
  return $retval;
        ...
        {	// New event - initialise everything
            $sql->db_Select("news", "*", "news_id='".intval($qs[1])."' ");
            ...
            $smarray = getdate($ne_datestamp);
            $month = $smarray['mon'];
            $year = $smarray['year'];
            $ne_startdate = $ecal_class->full_date($ne_datestamp);
            ...
            $end_hour = $smarray['hours'];
            $end_minute = $smarray['minutes'];
            $ne_enddate = $ecal_class->full_date($ne_datestamp);
			
			$ne_event = preg_replace('/request\.php/', '{e_BASE}request.php', $ne_event);
        }
...
//			alert('Format: ' + arrstr + '    Start date: '+ sdate + '    End date: ' + edate);
		...
		$text .= "
		<form method='post' action='".e_PLUGIN."calendar_menu/event.php' id='linkform' onsubmit='return calcheckform(this, submitted,{$ecal_class->java_format_code})'>
