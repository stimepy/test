...
while ($row = $sql->db_Fetch()) {
	if($row['link_parent'] == 0) {
		...
		$linklist['sub_'.$row['link_parent']][] = $row;
	}
}
$text = "<div class='pridebar'>
		<ul>";
...
foreach ($linklist['head_menu'] as $lk) {
	...
	if (isset($linklist['sub_'.$main_linkid])) {  // Has Children.
		$text .= "
			<li>".adnav_cat($lk['link_name'], (($lk['link_url']=='' || $lk['link_url']==NULL) ? e_SELF.'?'.e_QUERY.'#': $lk['link_url']), $main_linkid)."";

		$text .= "
				<ul>".render_sub($linklist, $main_linkid)."
				</ul>
			</li>";
		...
		// Display Parent only.
		$text .= "
			<li>".adnav_cat($lk['link_name'], $lk['link_url'], FALSE, $lk['link_open'])."</li>";
	}
