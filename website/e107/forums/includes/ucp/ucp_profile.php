...
class ucp_profile
{
	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			case 'reg_details':
				...
				if ($submit)
				{
					...
					if (!sizeof($error))
					{
						...
						//e107 information that is needed
						$e107_ary = array(
							'user_email'		=> $sql_ary['user_email'],
							'user_password'		=> $sql_ary['user_password'],
						);
						$sql_e107 ='UPDATE e107_user
								SET ' . $db->sql_build_array('UPDATE', $e107_ary) . '
								WHERE user_id = '.USERPHPBBID;
							$db->sql_query($sql_e107);
						}
			...
			case 'avatar':
                ...
                $this->ProfileAvatar($submit, $error);
    ...
    function ProfileAvatar($submit, &$error){
        global $config, $user, $auth, $template, $phpbb_root_path, $phpEx;
