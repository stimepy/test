...
class custom_profile
{
	...
	*/
	function generate_profile_fields_template($mode, $user_id = 0, $profile_row = false)
	{
		...
		{
            //Begin Xfire Mod
            require_once($phpbb_root_path.'includes/X_S_intergration.php');
            $xs = new xsintergration;
            //End Xfire Mod  */
			...
			foreach ($profile_row as $ident => $ident_ary)
			{
                ...
                $yesback=true;
                $otherinfo=false;
                if(preg_match('#^xfire_?[_a-z]*#',$ident_ary['data']['field_name'])){
                    if($ident_ary['data']['field_name']=='xfire'){
                        $yesback=false;

                    }
                    elseif($ident_ary['data']['field_name']=='xfire_style' ){
                        $value=$xs->determinexfiresty($value);

                        $yesback=false;
                    }
                    elseif($ident_ary['data']['field_name']=='xfire_disp'){
                        list($value, $sizew, $sizeh)=explode('.', $xs->determinxfiredis($value));
                        $sizes= array('PROFILE_XW'=>$sizew, 'PROFILE_XH'=>$sizeh);
                        $yesback=false;
                        $otherinfo = true;
                    }
                    if($xs->determinxfirok()){
                        $tpl_fields['blockrow'][] = array(
                            'S_PROFILE_XFIRE' => true
                        );
                    }
                }
                //End Xfire Mod
                // Begin Steam Mod
                if(preg_match('#^steam_id#',$ident_ary['data']['field_name'])){
                        if(!is_array($user_id)){
                            //trigger_error('User information in incorrect format.');
                            break;
                        }

                    include_once($phpbb_root_path.'includes/steamAPI/steamAPI.class.php');
                    include_once($phpbb_root_path.'includes/steamAPI/stm_sig.class.php');
                    $steam = new SteamAPI($value, $user_id);
                    $sig = new stm_sig($phpbb_root_path.'images/steam/stm_bakground.png', $steam);
                    $value=$sig->createSignature();
                }
                // end Steam Mod  */
                ...
                //Begin Xfire Mod
                if($otherinfo){
                    $tpl_fields['row'] = array_merge($tpl_fields['row'],$sizes);
                }
