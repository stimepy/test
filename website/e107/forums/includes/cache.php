...
class cache extends acm
{
	...
	*/
	function obtain_config()
	{
		...
		{
			$sqlA = 'SELECT config_name, config_value
				FROM ' . CONFIG_TABLE . '
				WHERE is_dynamic = 1';
		...
		else
		{
			...
			$sqlz = 'SELECT config_name, config_value, is_dynamic
				FROM ' . CONFIG_TABLE;
			$result = $db->sql_query($sqlz);
