	...
	if ($sql->db_Select_gen($query)) {
		...
		while ($row = $sql->db_Fetch()) {
			...
			if (check_class($forum_class)) 
			{
				if($user_name)
				{
					$POSTER = e107UserUrl($thread_user, $user_name);
					//user.php?id.
				} else 
				{
				...
				if (!$lastpost_id) {
					...
					$LASTPOST = e107UserUrl($lastpost_id, $lastpost_name)."<br />".$lastpost_datestamp;
					//user.php?id.
				}
...
if ($action == "top") {
	...
	if ($subaction == "forum" || $subaction == "all") {
		...
		while ($row = $sql->db_Fetch()) {
			...
			$text .= "<tr>
				...
				<td style='width:50%' class='forumheader3'>".e107UserUrl($user_id, $user_name)."</td>
	...
	if ($subaction == "chat" || $subaction == "all") {
		...
		while ($row = $sql->db_Fetch()) {
			...
			$text .= "<tr>
				<td style='width:10%; text-align:center' class='forumheader3'>{$counter}</td>
				<td style='width:50%' class='forumheader3'>". e107UserUrl($user_id, $user_name, e_HTTP) ."</td>
