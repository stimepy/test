...
//
	...
	function Mark_Query($query, $rli, $origQryRes, $aTrace, $mytime, $curtable) {
		...
		$nFields=0;
		$bExplained = FALSE;
		if (!strcasecmp($qtype,'SELECT') || !strcasecmp($qtype,'(SELECT')) { //if =0 then explain
            $nFields=$sql->db_QueryFreeForm("EXPLAIN {$query}", "fieldcount");  //query the explain
			$bExplained = TRUE;
            //todo finish 11/20/2012
		...
		$t['error']=$sQryRes ? '' : $sql->getSqlErrorno();
		$t['nFields']=$nFields;
		$t['time']=$mytime;

		if ($bExplained) {
			$bRowHeaders=FALSE;
			while ($row = $sql->db_QueryFreeForm('FETCH_A',$sQryRes)) {
				if (!$bRowHeaders) {
					$bRowHeaders=TRUE;
