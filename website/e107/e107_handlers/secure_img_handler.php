...
class secure_image
{
	...
	function render($qcode)
	{
        ...
        //@mysql_select_db($this->MYSQL_INFO['db']);  // no longer needed

		$result = $sql->db_Select_gen("SELECT tmp_info FROM {$this->MYSQL_INFO['prefix']}tmp WHERE tmp_ip = '{$recnum}'");
		if(!$result || !($row = $sql->db_Fetch(MYSQL_ASSOC))) // mysql_fetch_array($result, MYSQL_ASSOC
		{
			// echo "Render Failed";
