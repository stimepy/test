...
class userlogin {
	function userlogin($username, $userpass, $autologin) 
	{
		...
		*/
		...
		/*if (!$sql->db_Select("user", "*", "lower(user_loginname) = lower('".$tp -> toDB($username)."')"))
		{
        ...
        if(!$sql->db_Select("user", "*", "lower(user_loginname) = lower('".$tp -> toDB($username)."') AND user_password = '{$userpass}'")) {
			define("LOGINMESSAGE", LAN_300."<br /><br />");
			return FALSE;
		}
		else if(!$sql->db_Select("user", "*", "lower(user_loginname) = lower('".$tp -> toDB($username)."') AND user_password = '{$userpass}' AND user_ban!=2 ")) {
