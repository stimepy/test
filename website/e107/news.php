...
{	// --> Cache
	...
	if ($action == 'all')
	{
		...
		$query = "SELECT n.*, u.user_id, u.username user_name, r.rank_title user_customtitle, nc.category_name, nc.category_icon FROM #news AS n
		LEFT JOIN #phpbb_users AS u ON n.news_author = u.user_id
		LEFT JOIN #news_category AS nc ON n.news_category = nc.category_id
		LEFT JOIN #phpbb_ranks AS r ON r.rank_id=u.user_rank
...
{	// --> Cache
	...
	{
		$query = "SELECT COUNT(tb.trackback_pid) AS tb_count, n.*, u.user_id, u.username user_name, r.rank_title user_customtitle, nc.category_name, nc.category_icon FROM #news AS n
			LEFT JOIN #phpbb_users AS u ON n.news_author = u.user_id
			LEFT JOIN #news_category AS nc ON n.news_category = nc.category_id
...
switch ($action)
{
  ...
  default :
	...
	if(isset($pref['trackbackEnabled']) && $pref['trackbackEnabled']) 
	{
		$query = "SELECT COUNT(tb.trackback_pid) AS tb_count, n.*, u.user_id, u.username user_name, r.rank_title user_customtitle, nc.category_name, nc.category_icon, COUNT(*) AS tbcount FROM #news AS n
		LEFT JOIN #phpbb_users user AS u ON n.news_author = u.user_id
		LEFT JOIN #news_category AS nc ON n.news_category = nc.category_id
		LEFT JOIN #trackback AS tb ON tb.trackback_pid  = n.news_id
		LEFT JOIN #phpbb_ranks AS r ON r.rank_id=u.user_rank
		WHERE n.news_class REGEXP '".e_CLASS_REGEXP."' AND NOT (n.news_class REGEXP ".$nobody_regexp.")
...
function renderCache($cache, $nfp = FALSE){
	...
	global $db_debug,$ns,$eTraffic,$eTimingStart, $error_handler, $sql2, $mySQLserver, $mySQLuser, $mySQLpassword, $mySQLdefaultdb,$e107;
	echo $cache;
