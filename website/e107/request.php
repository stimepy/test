...
if ($type == "file")
{
	...
	if ($sql->db_Select_gen($qry)) 
	{
		...
		if (check_class($row['download_category_class']) && check_class($row['download_class'])) 
		{
			...
			{
				$bid = $result[1];
				if($sql->db_Select("rbinary", "*", "binary_id = '{$bid}'")){
                    $result=$sql->db_Fetch();

                    $binary_data = $result['binary_data'];//binary_data
                    $binary_filetype = $result['binary_filetype'];
                    $binary_name = $result['binary_name'];//binary_name
                    $sql->freeQuery();
                }
				//@mysql_query("SELECT * FROM ".MPREFIX."rbinary WHERE binary_id = '{$bid}'");
				//$binary_data = mysqli_result::field_seek ( int $fieldnr )
				//@mysql_result($result, 0, "binary_data");
	...
	else if(strstr(e_QUERY, "pub_"))
	{
		...
		if($sql->db_Select("rbinary", "*", "binary_id = '{$bid}' "))
		{
			...
			//$binary_filetype = @mysql_result($result, 0, "binary_filetype");
			//$binary_name = @mysql_result($result, 0, "binary_name");
			header("Content-type: {$result['binary_filetype']}");
			header("Content-length: {$download_filesize}");
			header("Content-Disposition: attachment; filename={$result['binary_name']}");
			header("Content-Description: PHP Generated Data");
			echo $result['binary_data'];
			exit();
		}
	}
...
{
	$bid = $result[1];
	$sql->db_Select("rbinary","*","binary_id = '{$bid}'");
    $result = $sql->db_Fetch();
	/*$binary_data = @mysql_result($result, 0, "binary_data");
	$binary_filetype = @mysql_result($result, 0, "binary_filetype");
	$binary_name = @mysql_result($result, 0, "binary_name");*/
	header("Content-type: {$result['binary_filetype']}");
	header("Content-Disposition: inline; filename={$result['binary_name']}");
	echo $result['binary_data'];
	exit();
}
