...
if (isset($_POST['update_cat'])) {
        $message = ($sql->db_Update("aacgc_roster_adv_cat", "cat_name='".$_POST['cat_name']."' WHERE cat_id='".$_POST['id']."' ")) ? "Successful updated" : "Update failed";
}

if (isset($_POST['main_delete'])) {
        $delete_id = array_keys($_POST['main_delete']);
	$sql2 = new db;
    $sql2->db_Delete("aacgc_roster_adv_cat", "cat_id='".$delete_id[0]."'");
...
if ($action == "") {
        ...
        <tr>
        <td style='width:' class='forumheader3'>Category ID</td>
        ...
        <td style='width:25%' class='forumheader3'>".$row['cat_name']."</td>
...
if ($action == "edit")
{
                $sql->db_Select("aacgc_roster_adv_cat", "*", "cat_id = $id");
                $row = $sql->db_Fetch();
...
";
        ...
        ".$rs -> form_close()."
	      ...
	      $ns -> tablerender("AACGC Advanced Roster (Edit Category)", $text);
	      require_once(e_ADMIN."footer.php");
}
?>
