...
/*######################################################
#Class: X1LadderMod  (Version 1.0.1)   
...
######################################################*/
class X1LadderMod implements X1EventMods{
...
#######################################*/	     
	public function X1SetChallenge($maps, $dates, $randid, $challenger, $challenged, $event, $rchallenged='', $rchallenger='')				
	{
		...
		if ($rchallenged['rung'] < $rchallenger['rung'] - $event['score'] ){
			$c .= laddermod_toomanyrungs.$event['score']." ".laddermod_toomanyrungs2."<br />";
			$c .= "Challenger Rung:: ".$rchallenger['rung']."<br/>";
			$c .= "Challenged Rung:: ".$rchallenged['rung']."<br/>";
...
#######################################*/		
    ...
    if(isset($_REQUEST['limit'])){
		...
		$pages = DispFunc::X1Pagination($numberofplayersin, X1_topteamlimit, 'limit', $cur_page, 'page', X1_publicpostfileX1_linkactionoperator."ladderhome&sid=$sid");
...
#######################################*/	
	public function X1DataInsert($edit=false){
		if(!$edit){
			$db_string1="score,ratings";
			$db_string2="".MakeItemString($_POST['score']).",	".MakeItemString($_POST['ratings'])."";
			return array($db_string1,$db_string2);
		}
		else{
			$db_string1="score=".MakeItemString($_POST['score']).", ratings=".MakeItemString($_POST['ratings'])."";
